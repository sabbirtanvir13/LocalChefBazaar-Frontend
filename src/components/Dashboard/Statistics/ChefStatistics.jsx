...
const ChefStatistics = () => {
  ...
  const { myMeals, myOrders, delivered, earnings } = data
  ...
  return (
    <div className="mt-12 px-2 lg:px-0">
      ...
      <div className="mb-12 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard title="My Meals" value={myMeals} icon={<FaUtensils />} color="from-purple-500 to-indigo-500" />

        <StatCard title="Total Orders" value={myOrders} icon={<BsFillCartCheckFill />} color="from-blue-500 to-cyan-500" />

        <StatCard title="Delivered" value={delivered} icon={<FaTruck />} color="from-green-500 to-emerald-500" />

        <StatCard title="Total Earnings" value={`$${earnings}`} icon={<FaDollarSign />} color="from-orange-500 to-amber-500" />
...
const StatCard = ({ title, value, icon, color }) => (
  <div className="relative bg-white rounded-xl shadow-md p-5">
    <div className={`absolute -top-6 left-5 w-12 h-12 rounded-xl bg-gradient-to-r ${color} flex items-center justify-center text-white`}>
      {icon}
    </div>
    <p className="text-sm text-gray-500 mt-6">{title}</p>
